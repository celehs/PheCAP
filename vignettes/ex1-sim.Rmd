...
head(validation$valid_roc) # ROC table: validation data
head(validation$train_roc) # ROC table: training data
head(validation$split_roc) # ROC table: random splits
validation
```
...
head(phenotype) # predicted probability
```
